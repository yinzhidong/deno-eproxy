...
function handleWebSocket(request, env) {
  ...
  const targetUrl = `wss://api.x.ai${pathAndQuery}`;
  ...
  targetWebSocket.addEventListener("open", () => {
    console.log('Connected to Grok server');
  ...
  proxy.addEventListener("message", async (event) => {
    ...
    if (targetWebSocket.readyState === WebSocket.OPEN) {
      try {
        ...
        console.log('Successfully sent message to Grok');
      } catch (error) {
  ...
  targetWebSocket.addEventListener("message", (event) => {
    console.log('Received message from Grok:', {
      dataPreview: typeof event.data === 'string' ? event.data.slice(0, 200) : 'Binary data',
      dataType: typeof event.data,
      timestamp: new Date().toISOString()
    });
  ...
  targetWebSocket.addEventListener("error", (error) => {
    console.error('Grok WebSocket error:', {
